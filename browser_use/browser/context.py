...
class BrowserContext:
	...
	async def take_screenshot(self, full_page: bool = False) -> str:
		...
		# Compress the screenshot to ensure it stays under 5MB
		import io
		...
		try:
			...
			# Start with reasonable compression settings
			quality = 85
			...
			while True:
				...
				if quality > 30:
					...
					# If we can't compress further, resize the image
					width, height = img.size
					img = img.resize((int(width * 0.9), int(height * 0.9)), Image.Resampling.LANCZOS)
					quality = 85  # Reset quality for the resized image
